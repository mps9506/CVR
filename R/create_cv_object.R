...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$grants  <- read_gsheet(sheet_id = "grants")
...
sanitize_links <- function(cv, text){
  if(cv$pdf_mode){
    link_titles <- stringr::str_extract_all(.data$text, '(?<=\\[).+?(?=\\])')[[1]]
    link_destinations <- stringr::str_extract_all(.data$text, '(?<=\\().+?(?=\\))')[[1]]
...
>{skill}</div>"
  ...
  cv$skills %>%
    dplyr::mutate(width_percent = round(100*as.numeric(.data$level)/out_of)) %>%
